                         ...
                         uint64_t* __restrict ob2) noexcept
{
    ...
    for (; w + 2 <= wpr - 1; w += 2) {
        ...
        // SRI / SLI fuse "shift + OR-in carry from neighbour word" into one op:
        //   (C<<k) | (L>>(64-k))  ≡  vsriq_n(C<<k, L, 64-k)    [insert L's top into low]
        //   (C>>k) | (R<<(64-k))  ≡  vsliq_n(C>>k, R, 64-k)    [insert R's low into top]
        // Saves one ALU op per shifted column → 4 ops/iter for the horizontal sum.
        const u64v col_m2 = vsriq_n_u64(vshlq_n_u64(C, 2), L, 62);
        const u64v col_m1 = vsriq_n_u64(vshlq_n_u64(C, 1), L, 63);
        const u64v col_0  = C;
        const u64v col_p1 = vsliq_n_u64(vshrq_n_u64(C, 1), R, 63);
        const u64v col_p2 = vsliq_n_u64(vshrq_n_u64(C, 2), R, 62);
        u64v b0, b1, b2;
                            ...
                            u64v& oe, u64v& oj, u64v& oa) noexcept
{
    ...
    // Transition tests on the box sum (5-bit) — algebraically reduced forms
    // (verified exhaustively for boxsum ∈ [0,31] against the original K-maps):
    //   birth   ∈ {3,4,5}:    (m1^m2) & (m2|m0) & ~m3 & ~m4              (5 ops)
    //   survive ∈ {5,…,10}:   (m3^m2) & ((m1^m3)|(m0^m3)) & ~m4          (6 ops)
    // The trick for the survive XORs: m3=0 ⇒ (m1^m3)|(m0^m3) = m1|m0;
    // m3=1 ⇒ (m1^m3)|(m0^m3) = ~m1|~m0 = ~(m1&m0).  One BSL collapsed into
    // two XORs and an OR — no explicit NOT needed.
    const u64v birth_inner  = v_and(v_xor(m1, m2), v_orr(m2, m0));
    const u64v birth        = v_bic(v_bic(birth_inner, m3), m4);

    const u64v survive_inner =
        v_and(v_xor(m3, m2),
              v_orr(v_xor(m1, m3), v_xor(m0, m3)));
    const u64v survive       = v_bic(survive_inner, m4);

...
// ─────────────────────────────────────────────────────────────────────────────
// One-vector compute + STNP non-temporal pair store
// ─────────────────────────────────────────────────────────────────────────────

struct CellPair { uint64x2_t b0, b1; };

// Compute (new_b0, new_b1) for one 128-cell NEON vector at position v.
static inline CellPair compute_vec(const uint64_t* const b0_in[5],
                                    const uint64_t* const b1_in[5],
                                    int v, int nvec) noexcept
{
...
// AArch64 STNP Q: store a pair of 16-byte NEON vectors (32 bytes total) to
// consecutive memory non-temporally.  Skips the write-allocate cache fill
// that a regular STR Q would trigger.  Requires 16-byte alignment of ptr.
static inline void stnp_q_pair(uint64x2_t v0, uint64x2_t v1, uint64_t* ptr) noexcept
{
    asm volatile("stnp %q1, %q2, [%0]"
                 :
                 : "r"(ptr), "w"(v0), "w"(v1)
                 : "memory");
}

// advance_row: produce one output row.  Processes 2 NEON vectors per iteration
// so the pair of consecutive output stores can be folded into one STNP Q (in
// the streaming path) or one STP Q (in the non-streaming path) at gcc's option.
...
static void advance_row(const uint64_t* const b0_in[5],
                        ...
                        int nvec) noexcept
{
    // nvec is a power of two ≥ 4 for every supported grid size (size ≥ 512).
    for (int v = 0; v < nvec; v += 2) {
        const CellPair pa = compute_vec(b0_in, b1_in, v,     nvec);
        const CellPair pb = compute_vec(b0_in, b1_in, v + 1, nvec);

        if constexpr (STREAMING) {
            stnp_q_pair(pa.b0, pb.b0, out_b0 + 2*v);
            stnp_q_pair(pa.b1, pb.b1, out_b1 + 2*v);
        } else {
            vst1q_u64(out_b0 + 2*v,     pa.b0);
            vst1q_u64(out_b0 + 2*v + 2, pb.b0);
            vst1q_u64(out_b1 + 2*v,     pa.b1);
            vst1q_u64(out_b1 + 2*v + 2, pb.b1);
        }
    }
}

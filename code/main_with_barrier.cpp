...
struct ThreadPool {
    ...
    // Initialize the barrier with our single-threaded completion phase action
    std::barrier<BarrierPhaseCompletion> b{NUM_THREADS, BarrierPhaseCompletion{}};
};

// Forward declaration so initialize_threads can see it
...
void initialize_threads(ThreadPool &pool) {
    ...
    for(int t = 0; t < NUM_THREADS; t++) {
        // Pass pool using std::ref to avoid copy-constructor errors
        pool.workers.emplace_back(step_thread, t * rows_per_thread, std::ref(pool));
    }

    // Block the main thread until all worker threads finish all generations
...
int main(int argc, char* argv[])
{
  ...
  BitGrid grid_b(grid_size);
    ...
    // Assign our global pointers before starting the threads!
    src = &grid_a;
    dst = &grid_b;
    ...
    // Unpack using 'src' (which correctly holds the final computed generation)
